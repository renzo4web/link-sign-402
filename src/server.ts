...
app.use(
  paymentMiddleware(
    {
      'POST /api/create': {
        accepts: [{ scheme: 'exact', price: config.payment.createPrice, network: networkCaip2, payTo }],
...
app.post('/api/create', async (c) => {
  ...
  customLogger(`[create] ${fileName} (${fileBytes.length}b) by ${creatorAddress.slice(0, 6)}… (${requestId})`)
  ...
  try {
    ...
    customLogger(`[create] ipfs failed: ${err instanceof Error ? err.message : err} (${requestId})`)
  ...
  const chainRef = config.blockchain.chainRef || networkToChainRef(config.blockchain.network)
